...
const BreweryApp = ({ 
  ...
  const handleScheduleBrew = (recipeId: string) => {
    ...
    updateData({ scheduledBrews: [...scheduledBrews, newBrew] });
  ...
  const handleScheduleShift = (username: string, type: "day" | "night") => {
      ...
      updateData({ workShifts: [...workShifts, newShift] });
      setSelectedDateForEvent(null); 
  };
  ...
  const toggleTask = (id: string) => {
      updateData({ tasks: tasks.map(t => t.id === id ? { ...t, completed: !t.completed } : t) });
  };
  ...
  const addTask = () => {
      ...
      updateData({ tasks: [newTask, ...tasks] });
      setNewTaskText("");
      ...
      <main className="flex-1 p-4 md:p-8 overflow-x-hidden w-full relative">
        ...
        {activeTab === 'integrations' && (
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 flex flex-col items-center text-center space-y-4">
                     <div className="bg-blue-900/30 p-4 rounded-full"><Key className="w-8 h-8 text-blue-500" /></div>
                     <h3 className="text-lg font-bold text-white">Экспорт базы (Ключ)</h3>
                     <p className="text-gray-400 text-xs">Создать ключ для переноса базы на другое устройство.</p>
                     <button onClick={handleExportDatabase} className="mt-auto bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-sm w-full">Сгенерировать ключ</button>
                </div>
                <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 flex flex-col items-center text-center space-y-4">
                     <div className="bg-purple-900/30 p-4 rounded-full"><Download className="w-8 h-8 text-purple-500" /></div>
                     <h3 className="text-lg font-bold text-white">Импорт базы</h3>
                     <p className="text-gray-400 text-xs">Ввести ключ для обновления данных.</p>
                     <input 
                        type="text" 
                     ...
                     />
                     <button onClick={handleImportDatabase} className="mt-auto bg-purple-600 hover:bg-purple-500 text-white px-4 py-2 rounded-lg text-sm w-full">Импортировать</button>
                </div>
             </div>
        )}
...
const Root = () => {
  ...
  useEffect(() => {
    ...
    if (session) {
        try {
            ...
            if (brewery && username) {
                const storedDb = localStorage.getItem(`brewery_db_${brewery}`);
                if (storedDb) {
                     const data = JSON.parse(storedDb) as BreweryData;
  ...
  const handleAuth = (e: React.FormEvent) => {
      ...
      const dbKey = `brewery_db_${formData.brewery}`;
      const storedData = localStorage.getItem(dbKey);

      if (authMode === "register") {
          if (storedData) {
              setError("Пивоварня с таким названием уже существует на этом устройстве.");
              return;
          }

          const newAdmin: UserAccount = { username: formData.user, password: formData.password, role: "admin" };
          const newData: BreweryData = { ...INITIAL_DATA, users: [newAdmin] };
          
          localStorage.setItem(dbKey, JSON.stringify(newData));
          setBreweryData(newData);
          ...
          // Login
          if (!storedData) {
              setError("Пивоварня не найдена на этом устройстве. Попросите администратора передать вам Ключ Базы Данных.");
              return;
          }

          const data = JSON.parse(storedData) as BreweryData;
          const user = data.users.find(u => u.username === formData.user && u.password === formData.password);
  ...
  const handleUpdateData = (newData: Partial<BreweryData>) => {
      ...
      const dbKey = `brewery_db_${formData.brewery}`;
      localStorage.setItem(dbKey, JSON.stringify(updatedData));
  };
